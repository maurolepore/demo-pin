...
```{r, message=FALSE}
my_data <- read_csv(pin(raw_url, name = "my_data", board = "my_local_board"))
fs::dir_tree(my_cache)
```
...
It is not pins job to control who reads or writes data but it plays well with tools like GitHub that provide granular [access permissions](https://help.github.com/en/github/getting-started-with-github/access-permissions-on-github).
